...
const namespaces: NamespaceDefinition[] = [
  {
    ...
    assetPattern: taxonomyIconAssetPattern,
  },
  {
    slug: SITE_BRAND_LOGO_SLUG,
    assetPattern: siteBrandLogoAssetPattern,
  },
  {
    slug: SITE_BACKGROUND_MEDIA_SLUG,
    assetPattern: siteBackgroundAssetPattern,
  },
];
...
async function scanNamespace(
  ...
  for (const entry of entries) {
    ...
    if (!definition.assetPattern.test(publicPath)) {
      unexpected.push(filePath);
      continue;
    }
...
async function runSelfTest() {
  ...
  try {
    ...
    const backgroundPath =
      `/media/editorial/${SITE_BACKGROUND_MEDIA_SLUG}/${"e".repeat(64)}.webp`;
