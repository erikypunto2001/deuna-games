...
export async function POST(
  ...
  if (
    ...
    return adminRedirect(
      authorized.adminOrigin,
      `${target}&estado=historial-confirmacion`
    );
  }
  ...
  try {
    ...
    const result =
      await compactGameEditorialHistory(
        slug,
        authorized.session.userId,
        {
    ...
    if (result.outcome === "conflict") {
      return adminRedirect(
        authorized.adminOrigin,
        `${target}&estado=historial-conflicto`
      );
    }

    return adminRedirect(
      authorized.adminOrigin,
      `${target}&estado=historial-limpiado`
    );
  } catch {
    console.error(
      "No se pudo limpiar el historial completo del juego."
    );
