...
CREATE OR REPLACE FUNCTION deuna_admin.is_game_media_cleanup_pending(
  p_game_slug text
)
RETURNS boolean
LANGUAGE sql
...
CREATE OR REPLACE FUNCTION deuna_admin.prevent_game_recreation_during_media_cleanup()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, deuna_admin
...
BEGIN
  IF NEW.item_type = 'game'
     AND EXISTS (
       SELECT 1
         FROM deuna_admin.game_media_cleanup_queue
        WHERE game_slug = NEW.item_key
     ) THEN
    RAISE EXCEPTION
      'game media cleanup pending for slug %',
      NEW.item_key
...
DROP TRIGGER IF EXISTS editorial_items_block_pending_media_cleanup
  ON deuna_admin.editorial_items;

CREATE TRIGGER editorial_items_block_pending_media_cleanup
BEFORE INSERT ON deuna_admin.editorial_items
FOR EACH ROW
EXECUTE FUNCTION deuna_admin.prevent_game_recreation_during_media_cleanup();

CREATE OR REPLACE FUNCTION deuna_admin.list_game_media_cleanup_queue(
